<template>
  <div class="login-container">
    <el-alert v-if="showAlert"
      title="您正在使用的IE内核浏览器兼容性差，网页加载慢，为了您有更好的操作体验，请下载使用谷歌浏览器，如使用的是360浏览器、百度浏览器、搜狗浏览器、QQ浏览器等，请切换到极速模式。" type="error"
      show-icon />
      ...
      <el-card class="box-card" style="margin:30px auto; width: 400px;">
        ...
        <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on"
          ...
          <el-form-item prop="password">
            <el-input ref="password" v-model="loginForm.password" placeholder="请输入密码" show-password auto-complete="off"
...
export default {
  ...
  created() {
    this.systemName = settings.title
  },
  methods: {
    handleLogin() {
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          store.login(this.loginForm).then(() => {
            this.loading = false
